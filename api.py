...
def get_urls():
    try:
        ...
        cursor.execute("SELECT url, dom FROM urls")
        ...
        urls_list = [{'url': row[0], 'dom': row[1]} for row in rows]
...
def get_urls_without_dom():
    try:
        ...
        # Récupération des URLs à partir de la table "urls"
        ...
        # Conversion des résultats en une liste de chaînes
        ...
        print("Erreur lors de la récupération des URLs :", error)
        return jsonify({'error': 'Erreur lors de la récupération des URLs'})
